...
//==================================================================================================
//  1) DESCRIPTION
//    buildPositionTree_Master — Phase A of the master-games position-tree pipeline. Mirrors
//    buildPositionTree_Player.ts's buildPositionTree_Player exactly, minus the player filter
//    (Step 3 is global — every master player's outstanding games, not one at a time) and minus
//    the gd_positions_purged exclusion (no purge feature exists for master games). Replays each
//    fetched game's PGN with chess.js and writes one tmgam_game_positions row per recordable ply
//    (insertGamePositions_Master), then (unless skipSync) hands off to syncTposFromTgam_Master
//    (Phase B, exported separately below) to derive/backfill tmpos_positions from what was just
//    written.
//
//    Parameters:
//      opts.limit       — max games to process this run (default POSITION_TREE_LIMIT_Master)
...
//      opts.playerLabel — display-only tag for the logged step name — no filtering effect,
//                         Step 3 stays global
//
//    Returns:
//      gamesProcessed — games fetched this run
//      positions      — recordable positions written (tmgam_game_positions rows with moveNum > 0)
