...
import {
  ...
  DEFAULT_GRAPH_LIMIT,
  DEFAULT_GRAPH_GRANULARITY,
  DEFAULT_GRAPH_TIME_CLASS,
  DEFAULT_OPENINGS_SORT_FROM,
  DEFAULT_OPENINGS_SHOW,
...
const CONSTANTS_SECTIONS: ConstantSection[] = [
  {
    ...
    entries: [
      ...
      { name: 'DEFAULT_GRAPH_LIMIT', value: DEFAULT_GRAPH_LIMIT, description: "Default value for the Rating Graph page's Records dropdown; 0 = All (no fetch cap).", consumers: ['graph/page.tsx: GraphContent'] },
      { name: 'DEFAULT_GRAPH_GRANULARITY', value: DEFAULT_GRAPH_GRANULARITY, description: "Rating Graph's initial granularity override — Weekly. Falls back to the span-based auto pick when the fetched date span is too short for Weekly to be offered.", consumers: ['RatingChart.tsx: RatingChart'] },
      { name: 'DEFAULT_GRAPH_TIME_CLASS', value: DEFAULT_GRAPH_TIME_CLASS, description: "Fallback time class for the Rating Graph when ?timeClass= is unset or holds a value the current player doesn't play (both tracked players play blitz).", consumers: ['FilterGraphTimeClassSelect.tsx: FilterGraphTimeClassSelect'] },
      { name: 'DEFAULT_OPENINGS_SORT_FROM', value: DEFAULT_OPENINGS_SORT_FROM, description: "Openings chart 'From' default — which end of the score% ranking to show ('Best' or 'Worst'). Only applies to a fresh session; a persisted osc-from wins.", consumers: ['OpeningScoreChart.tsx: OpeningScoreChart'] },
      { name: 'DEFAULT_OPENINGS_SHOW', value: DEFAULT_OPENINGS_SHOW, description: "Openings chart 'Show' (row count) default; '0' = All. Only applies to a fresh session; a persisted osc-results-count wins.", consumers: ['OpeningScoreChart.tsx: OpeningScoreChart'] }
    ]
  },
  {
    ...
    entries: [
      ...
      { name: 'WIDTH_GRAPH_GRANULARITY', value: WIDTH_GRAPH_GRANULARITY, description: "Width for the Rating Graph's Granularity dropdown — set so 'Weekly Avg' / 'Monthly Avg' fit; without it the MySelect default w-72 made it far wider than the other graph filters.", consumers: ['RatingChart.tsx: RatingChart'] },
      ...
      { name: 'WIDTH_OPPONENT_RATING', value: WIDTH_OPPONENT_RATING, description: 'Shared width for the gd_opponent_rating FilterNumberRange (Games + Master Games filter rows).', consumers: ['GameList.tsx: GameList', 'MasterGameList.tsx: MasterGameList'] },
      ...
      { name: 'WIDTH_MASTER_PLAYER', value: WIDTH_MASTER_PLAYER, description: "Default dropdown width for MasterPlayerSelect (used both as the Masters Games list 'Player' filter, scope='synced', and generally for picking any known master, scope='all') — wider than WIDTH_PLAYER to fit longer master names.", consumers: ['MasterPlayerSelect.tsx: MasterPlayerSelect'] },
  ...
  },
  {
    ...
    entries: [
      ...
      { name: 'MOVE_COUNT_MIN_MOVE', value: MOVE_COUNT_MIN_MOVE, description: "The Analyze/master-analyze pages' ×N move-play-count badge/check only applies from this move number onward. Deliberately separate from MIN_ANALYSIS_MOVE_Player. Shared by both player and master views since both need the identical threshold.", consumers: ['ChessBoardView_shared.tsx: ChessBoardView_shared', 'MasterGameView_master.tsx: MasterGameView_master'] },
      ...
      { name: 'RESULT_MISMATCH_CP_THRESHOLD_Player', value: RESULT_MISMATCH_CP_THRESHOLD_Player, description: "How decisive gd_final_eval must be, in either direction, before a game's recorded result is flagged as contradicting its final position.", consumers: ['chessdb_player.ts: fetchGamesForPosition_player'] },
  ...
  },
  {
    ...
    entries: [
      ...
      { name: 'STOCKFISH_DEPTH_INPUT_MAX', value: STOCKFISH_DEPTH_INPUT_MAX, description: "Maximum value accepted by /analyze's Depth number inputs (Game Analysis and Stockfish panels) — typed values above this clamp down to it.", consumers: ['DepthInput_shared.tsx: DepthInput_shared'] }
    ]
  },
  {
    heading: 'UI Display',
...
const FUNCTION_DESCRIPTIONS: Record<string, string> = {
  ...
  'RatingChart.tsx: RatingChart': 'Rating-over-time line chart (one series per player for the selected time class), with a span-based granularity dropdown defaulting to Weekly.',
  ...
  'ChessBoardView_shared.tsx: ChessBoardView_shared': 'Interactive game analysis board — move tree, Stockfish batch/position analysis, and position-history panels for one game.',
  'DepthInput_shared.tsx: DepthInput_shared': "Shared Depth number input for /analyze's Game Analysis and Stockfish panels — types freely, clamps to min/max only on blur.",
  ...
  'chessdb_player.ts: fetchGamesForPosition_player': "Fetches one page of a player's games that reached a given position, optionally narrowed to a given move, with result-mismatch flags.",
  'chessdb_player.ts: getGamesForPositionCount_player': "Total row count for fetchGamesForPosition_player's same filter set, for pagination.",
  ...
  'MasterPlayerSelect.tsx: MasterPlayerSelect': "Master player picker, searchable — scope='synced' (Masters Games list Player filter) or scope='all' (every known master, e.g. the Chess.com Games search's Player 1/2); valueField picks chess.com handle or \"First Last\" name. Options sorted alphabetically by name, no grade shown.",
  ...
  'FilterGraphTimeClassSelect.tsx: FilterGraphTimeClassSelect': "Graph-only time-class picker: single-select, no 'All', player-aware option list (the selected player's getPlayerTimeClasses(), or the union across all tracked players), writing the shared ?timeClass= URL param.",
  ...
  'TerminationMultiSelect.tsx: TerminationMultiSelect': 'Reusable gd_termination multi-select checkbox group, options overridable per call site (GameList + MasterGameList).',
